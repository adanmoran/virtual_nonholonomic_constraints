%! TEX root = main.tex

%/========== Symbols ==========/%

% Set the heading so it appears in the Table of Contents
\addcontentsline{toc}{chapter}{List of Symbols}
\chapter*{List of Symbols}

% Define a table for the symbols using the booktabs convention
\noindent
\begin{table}[h!]
\resizebox{\textwidth}{!}{%
\centering
\begin{tabular}{ll}
   \toprule
   Symbol & Definition \\
   \midrule
   \(\n\) & The index set \(\{1,\ldots,n\}\) of natural numbers up to \(n\). \\
   \(\R^n\) & Real numbers in \(n\) dimensions. \\
   \(\Rt{T}\) & Real numbers modulo \(T > 0\), with \(\Rt{\infty} = \R\). \\
   \(\mathbb{S}^1\) & The unit circle, equivalent to \(\Rt{2\pi}\). \\
   \(\R^{n \times m}\) & The space of real-valued matrices with \(n\) rows and \(m\) columns. \\
   \(\Id{n}\) & The \(n \times n\) identity matrix. \\
   \(\Zmat{n \times m}\) & The \(n \times m\) matrix of all zeros. \\
   \(M_i\) & If \(M\) is a vector, the \(i\)th element of \(M\). \\
         & If \(M\) is a matrix, the \(i\)th column of \(M\). \\
   \(M_{i,j}\) & The value of row \(i\), column \(j\) for the matrix \(M\). \\
   \([v_i]\) & If \(v_i \in \R\), the column vector with value \(v_i\) at position \(i\). \\
      & If \(v_i \in \R^n\), the matrix with column \(i\) given by \(v_i\). \\
      & If \(v_i \in \R^{1\times n}\), the matrix with row \(i\) given by \(v_i\). \\
   \(\dot{x}\) & Derivative of \(x\) with respect to time \(t\). \\
   \(\nabla_v F\) & Gradient of the \(\R\)-valued function \(F\) with respect to \(v\). \\
   \(dF_v\) & Total differential (Jacobian) of the function \(F\) with respect to \(v\).\\
           & If \(F\) is \(\R\)-valued, equivalent to \((\nabla_v F)\tpose\). \\
   \(\nabla_v dF_w\) & Derivative matrix of the \(\R\)-valued function \(F\), with \((i,j)\) element \(\partial^2 F/(\partial v_i \partial w_j)\). \\
   \(\nabla_v^2 F\) & Hessian of \(F : \R^n \rightarrow R\), equivalent to \(\nabla_v dF_v\). \\
   \(\delta_{i,j}\) & The Kronecker delta: 1 if \(i = j\) and 0 otherwise. \\
   \bottomrule
\end{tabular}

}% resizebox
\end{table}

%---------- Nomenclature ----------%
%\chapter*{Nomenclature}
%
%% Define a table for the symbols using the booktabs convention
%\noindent
%\begin{table}[h!]
%\resizebox{\textwidth}{!}{%
%\centering
%\begin{tabular}{ll}
%   \toprule
%   Acronym & Meaning \\
%   \midrule
%   DOF & Degrees of freedom
%   EOC & Equation of constraint \\
%   EOM & Equation of motion \\
%   ODE & Ordinary differential equation \\
%   VNHC & Virtual nonholonomic constraint \\
%   WLOG & Without loss of generality \\
%   \bottomrule
%\end{tabular}
%
%}% resizebox
%\end{table}
%/========== /Symbols ==========/%
% vim: set ts=3 sw=3 sts=0 et ffs=unix :
