...
parameter \(I\) must be small.
While we could choose \(I\) arbitrarily small in simulations, we will eventually
compare these simulations with the physical experiments where \(I\) must be
large enough to overcome friction in leg motor.
Setting \(\bar{q}_a = 1\), we experimentally determined that \(I = 10\) is a
viable control value.
This is the value of \(I\) we used in all simulations and experiments.

\textbf{TODO: the injection and dissipation I is different, so we might not use
I = 10 for dissipation.}

\subsection{Energy Injection}

To validate the energy injection property of our VNHC, we simulated the acrobot
with initial condition \((q_u,p_u) = \left(\pi/32,0 \right)\) and plotted the
resulting orbit in Figure \ref{fig:acrobot-orbit}.
...
continuously. Here we only gain energy up to speed 0.195 because of friction.}
