...
\documentclass[journal,twoside,web, onecolumn,draftcls]{ieeecolor}
...
The terminology " gain energy" or "lose energy" is to be interpreted loosely in
this context.
...
That is, given a level set of \eqref{eqn:nominal-energy} contained in 
\(\mathcal{O}_2(\rho)\), one can stabilize a neighbourhood of this level set by
appropriately toggling between injection and dissipation VNHCs, which can be
achieved by changing the sign of \(I\) in \eqref{eqn:acrobot-constraint}.

In the rest of this article we distinguish between ``rotations" and
``oscillations".

%We say the acrobot is \textit{rotating} with \textit{rotation rate} \(p_0\)
%whenever \((q_u(t),p_u(t))\) crosses the \(p_u\)-axis at \(|p_u| = p_0\), 
%and \(p_u(t)\) has not changed sign since the previous time the orbit crossed
%the \(p_u\)-axis.
We say the acrobot is \textit{rotating} whenever \((q_u(t),p_u(t))\) crosses the 
\(p_u\)-axis, and \(p_u(t)\) has not changed sign since the previous time the
orbit crossed the \(p_u\)-axis.
The value of \(|p_u|\) at this crossing is the \textit{rotation rate}. 
If \(I = 0\) in \eqref{eqn:acrobot-constraint}, rotations are
closed periodic orbits of \((q_u(t),p_u(t))\) which are entirely contained in
\(\mathcal{R}(\rho)\), the rotation domain defined in
\eqref{eqn:rotation-domain}.
In this case, each rotation is contained in a level set of
\eqref{eqn:nominal-energy}, which is uniquely determined by its rotation rate.
\textbf{TODO: state that if abs(qu) = pi and pu neq 0, the acrobot must necessarily
rotate}

We say the acrobot is \textit{oscillating} if it is not rotating.
Since \(p_u(t)\) changes sign, an oscillating acrobot must necessarily cross the
\(q_u\)-axis at some point \(q \in \, ]-\pi,\pi[\).
The value of \(|q|\) at this crossing is called the \textit{oscillation angle}.
If \(I = 0\) in \eqref{eqn:acrobot-constraint}, oscillations are closed periodic
orbits of \((q_u(t),p_u(t))\) which are entirely contained in \(\mathcal{O}_1\).
In this case, each oscillation is contained in a level set of
\eqref{eqn:nominal-energy} which is uniquely determined by its oscillation
angle.
...
\end{itemize}
This method is entitled ``rotation regulation", which implies that one must choose
\(p_\text{des}\) and \(\delta\) so that 
\((1-\delta) p_\text{des} > \sqrt{60m^2gl^3}\).
that way, when \(q_a = 0\), the resulting orbit is a rotation.
In fact, it is entirely possible to choose \(p_\text{des}\) and \(\delta\) so
that \((1+\delta)p_\text{des} \leq \sqrt{60m^2gl^3}\).
Rather than stabilizing a rotation, the above method would stabilize an
oscillation whose momentum at the bottom of the swing arc is \(p_\text{des}\).

The problem with rotation regulation is that it assumes the supervisor can
instantaneously transition from an injection/dissipation VNHC (with non-zero \(I\))
to extending the leg by setting \(q_a = 0\).
This is impossible because the transition is happening on the \(p_u\)-axis.
If the supervisor changed the VNHC on the \(q_u\)-axis instead, 
this transition to \(q_a = 0\) can be done continuously.
Hence, we now present the preferred method for stabilizing oscillations.
...
whenever the acrobot begins rotating (i.e., when \(|q_u| = \pi\) and \(p_u
\neq 0\)), the supervisor does the following: 
\begin{itemize}
    \item If \(|q_u| < (1-\delta)q_\text{des}\), enable the injection VNHC by
        setting \(I > 0\) in the VNHC \eqref{eqn:acrobot-constraint}.
    \item If \(|q_u| > (1+\delta)q_\text{des}\), enable the dissipation VNHC by
        setting \(I < 0\) in the VNHC \eqref{eqn:acrobot-constraint}.
...
using a procedure devised by Mohammadi \etal
\cite{dynamic_vhcs_stabilize_closed_orbits}, on certain subsets 
\(D \subset \SxR\) one can find a diffeomorphism \(T(q_u,p_u) = (r,\theta)\)
...
\ref{thm:acrobot-energy-stabilization}, we begin by using the procedure from
\cite{dynamic_vhcs_stabilize_closed_orbits} to find 
