...
The sets \(\mathcal{O}_1\) and \(\mathcal{O}_2(\bar{\rho})\) are sublevel sets of
the nominal pendulum's energy \(E(q_u,p_u)\) described in
\eqref{eqn:acrobot-energy}.
To prove Theorem \ref{thm:acrobot-energy-stabilization}, we will find conditions
under which the acrobot gains energy on an open ``energy annulus".
That is, given two energy levels \(E_0 \geq 0\) and \(E_1 > E_0\), we define the
energy annulus to be the domain
\begin{equation*}
    \mathcal{D} := \{(q_u,p_u) \in \SxR \mid \\
    E_0 < E(q_u,p_u) < E_1\}
    .
\end{equation*}
The only condition we place on \(E_0\) and \(E_1\) is that they both correspond
to oscillations or both to rotations; that is, if \(E_0 < E(\pi,0)\), then 
\(E_1 \leq E(\pi,0)\).

Here is the fundamental idea of the proof: for certain open subsets
\(D \subset \mathcal{D}\), one can find a change of coordinates into 
...
We will describe the explicit forms of these transformations later. 
Importantly, we will show that the dynamics of \((r,\theta)\) satisfy 
\(\dot{r} = 0\) and \(\dot{\theta} > 0\) for the nominal pendulum (i.e., when
\(I = 0\)).
By continuity of the constrained dynamics with respect to \(I\), we can find a
small enough \(I > 0\) so that \(\dot{\theta} > 0\) for the acrobot.
...
Then we will find the relevant subset \(D \subset \mathcal{D}\)
and the coordinate transformations \((q_u,p_u) \mapsto (r,\theta)\) which
satisfy the conditions of the lemma, thereby proving the acrobot gains energy on
each set.
...
\begin{proof}
    ...
    Proving the constrained dynamics gain energy on \(D\) is equivalent to
    proving \(\hat{r}_{n}\) eventually reaches \(\overline{r}\).
    Thanks to a perturbation analysis from \cite[Theorem 10.1]{khalil_nonlinear}, 
    we know there exists some \(I_2 \in ]0,I_1]\) and \(\delta > 0\)
    such that \(\left|R(2\pi,r_n,I)\right| \leq \delta I^2\) for all 
    \(I \in [-I_2,I_2]\).
    Taking this fact alongside the assumption that \(S(r_0)\) is uniformly
    bounded below by \(\epsilon\), we can find a lower bound on
    ...
    Therefore, \(r_{n+1} \geq r_n + \gamma \geq r_0 + n\gamma\), which means all
    solutions of \eqref{eqn:r-discrete} flow towards \(\overline{r}\).
    We conclude that, for \(I \in ]0, I^\star]\), the constrained dynamics gain
    energy on \(D\).
    Changing the sign of \(I\) means \(r_{n+1} \leq r_n - n\gamma\), in which
    case the constrained dynamics lose energy on \(D\).
\end{proof}
\textbf{TODO: Prove origin is unstable, then apply lemma to O1 and O2 and show
that orbits go through Epi}
%/========== Conclusion ==========/%
