...
the highest impact on the magintude of energy injection. 
...
angle and velocity when performing giants, not as functions of time. 
Doing so allows them to gain energy and rotate around the bar.

The simplest way to model a gymnast is as a variable-length pendulum, but a
...
In this article, our goal is to design a leg controller which provably injects
energy into the acrobot, so that the resulting motion mimics that of a human
performing a giant.
...
trajectory tracking, or other feedback methods to stabilize
certain energy levels (see
...
While all these approaches result in the acrobot rotating around the
bar, none of them are particularly human-like.
After all, the results of \cite{pendulum_length_giant_gymnastics} indicate that
a robot's legs should track a function of its body angle and velocity.
In 2016, Wang attempted to design a controller with this in mind \cite{xingbo_thesis}
...
of virtual nonholonomic constraint.
...
Here then is the problem we will solve in this article: design a virtual
nonholonomic constraint so that the acrobot provably gains energy while
performing giant-like motion.
The following is a rigorous definition of ``energy gain".
...
where we now have \((q,p) = (q_u,q_a,p_u,p_a) \in \mathcal{Q}\times \R^n\).
...
For our model of the acrobot, we assume the acrobot is comprised of two massless
