...
%    ,
%\end{equation*}
%where \(E_1 > E_0\) are energy values, which we will choose later.
...
\begin{proof}
    ...
    The solution of \ref{eqn:r1-hat-prime} can be obtained by quadrature, giving
    \[
    ...
    Hence, we can understand the evolution of \(\hat{r}\) by studying
...
\ref{thm:acrobot-energy-stabilization}.
To apply Lemma \ref{lemma:energy-regulation-D}, we choose \(D\) to be the
...
one can verify that \(\dot{\theta} > 0\) for every \(r \in \, ]0,\pi[\).
Setting \(g(\theta,r,I) = \dot{r}/\dot{\theta}\), MATLAB symbolic computations
confirm that \((\partial g/\partial r)(\theta,r,0) \equiv 0\) as required.
Next, we compute
\begin{equation*}
    b(\theta,r_0) = 
    \frac{\partial g}{\partial I}(\theta, r_0, 0) = K a(\theta,r_0)
    ,
\end{equation*}
where
\begin{align*}
    ...
    .
\end{align*}
Notice that \(a(\theta,r_0)\) does not depend on the physical parameters of the
acrobot.

\textbf{TODO: write about Qro, show the figure, apply lemma to D,
    prove origin is unstable, show orbits will go through Epi, 
    then derive the constrained dynamics for the rotation domain and apply lemma
    there, concluding that O2 is O1 U Epi U rotation domain and is therefore
    also gaining energy.}
