...
A gymnast will begin (at rest) by hanging on a bar, then swing their legs
...
In other words, gymnasts were moving their legs as a function of their body's
angle and their angular velocity around the bar, not as functions of time.

Typically as control theorists we like our system's actuatiors to track a fixed
trajectory over time; but if we were attempting design a gymnastics robot that
...
\begin{figure}
    \centering
    %\includegraphics[width=\textwidth]{images/acrobot.png}
    \caption{The two-link acrobot.}
    \label{fig:acrobot}
\end{figure}
...
Previous approaches to solve this problem used
...
While they all result in the acrobot performing rotations on the bar, none of
the are particularly human-like.
The most realistic controller to date was designed by Wang \cite{xingbo_thesis}; it
forces the hips to track a function of the torso angle.
However, a truly human-like controller would force
the hips to track a function involving the torso velocity.
These kinds of controllers are known as ``virtual nonholonomic constraints", and
they have been used for human-robot interaction
...
Indeed, virtual nonholonomic constraints produce more robust bipedal motion than
...
Hence, we will design one of these to achieve stable giant motion on the
acrobot.

Here then is the problem we will solve: design a virtual nonholonomic constraint
for the acrobot, so that the acrobot provably gains energy over time
(as per Definition \ref{defn:energy-gain}).
