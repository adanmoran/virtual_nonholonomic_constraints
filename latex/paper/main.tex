...
of \(I\) should dissipate energy from the acrobot.
...
As is expected, the acrobot slows down over time.
...
To test how VNHCs can perform energy regulation, we employed a simple supervisor
which toggles between injection and dissipation VNHCs to stabilize an
orbit which crosses the \(q_u\)-axis at some desired angle (for oscillations) or
the \(p_u\)-axis at some desired velocity (for rotations).
Specifically, one chooses some appropriate energy value \(E_0 > 0\) and a hysteresis
percentage \(\delta \in [0,1]\).
The supervisor enables the injection VNHC if 
\(E(q_u,p_u) < (1-\delta)E_0\), the dissipation VNHC if 
\(E(q_u,p_u) > (1+\delta) E_0\), and it keeps the leg angle constant if
\((1-\delta)E_0 \leq E(q_u,p_u) (1+\delta)\leq E_0\).

First, we wanted the acrobot to oscillate with a maximum body angle 
\(q_u = \pi/2\).
The corresponding energy is \(E_0 = E(\pi/2,0)\), and we set our hysteresis to
be \(\delta = 5\%\).
\textbf{TODO: do the energy regulation experiment and see the results.}

\subsection{Summary of Results}
The simulation results in this section demonstrate the energy regulation
capabilities of our VNHC.  
In particular, Theorem \ref{thm:acrobot-energy-stabilization} appears to work
even for acrobots whose limbs have differing masses and lengths.
The energy regulation supervisor does not 
