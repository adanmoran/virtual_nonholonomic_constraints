...
Recall that we wish to design a VNHC which injects energy into the acrobot
through giant-like motion.
For our model of the acrobot, we assume the torso and leg rods are of equal
length \(l\) with equal point masses \(m\) at the tips, as in Figure
\ref{fig:simple-acrobot-model}.
Note that a real gymnast cannot swing their legs in full circles, though they
are usually flexible enough to raise them parallel to the floor; 
for this reason, we assume that \(q_a \in [-Q_a, Q_a]\) where 
\(Q_a \in [\frac{\pi}{2}, \pi[\). 
We also ignore any frictional forces at both the hip and shoulder joints. 
...
seated swing.
The person extends their legs when the swing moves forwards, and retracts them
when the swing moves backwards.
...
Here, \(\bar{q}_a \in ]0,2 Q_a/\pi]\) and \(I \in \R\) is a fixed
control parameter.
...
Hence, the constrained dynamics are the torso dynamics when one swings the legs
according to \eqref{eqn:acrobot-constraint}.
It is not hard to show that the constrained dynamics are
...
One can study the constrained system by examining solely the motion of 
\((q_u,p_u) \in \SxR\).
Suppose for a moment that \(I = 0\) -- that is, the legs stay fully extended.
The acrobot becomes a pendulum with two masses, whose total mechanical
energy is
\begin{equation}\label{eqn:acrobot-energy}
    E(q_u,p_u) := \frac{p_u^2}{10ml^2} + 3mgl(1 - \cos(q_u))
    .
\end{equation}
If the pendulum is oscillating, its phase lies in the domain
\begin{equation}\label{eqn:oscillation-domain}
    \mathcal{O}_1 := \left\{(q_u,p_u) \in \SxR 
    \mid E(q_u,p_u) < E(\pi,0) \right\}
    ,
\end{equation}
which is shown on the \((q_u,p_u)\)-plane in Figure 
\ref{fig:acrobot-oscillation-domain}.
If for some \(I \neq 0\) our VNHC injects energy into the acrobot on 
\(\mathcal{O}_1\), and additionally the constrained dynamics escape
\(\mathcal{O}_1\) in finite time, then we can guarantee the acrobot will perform
giant-like motion and begin rotating around the bar.

The upright equilibrium of the pendulum is at \((q_u,p_u) = (\pi,0)\).
If the pendulum were initialized at \(q_u = 0\), the minimum initial momentum
required to reach the upright equilibrium would be
\(p_u = \sqrt{60m^2gl^3}\), since \(E(\pi,0) = E(0,\sqrt{60m^2gl^3})\).
Hence, if the pendulum is rotating with momentum 
\(\sqrt{60m^2gl^3} < p_u < \bar{\rho}\),
the pendulum phase must lie inside
\begin{equation}\label{eqn:rotation-domain}
    \mathcal{R}(\rho) := \left\{
        (q_u,p_u) \in \SxR \mid E(\pi,0) < E(q_u,p_u) < E(0,\rho)
    \right\}
    .
\end{equation}
...
\(I \neq 0\), then the acrobot will swing up, begin rotating, and eventually
rotate with a momentum of at least \(\bar{\rho}\).
...
Unfortunately, the constrained dynamics do not always gain energy on
\(\mathcal{O}_1\) and \(\mathcal{O}_2(\bar{\rho})\);
energy gain is only guaranteed under certain conditions on the size of \(I\).
If \(I\) is too large, the leg controller saturates (i.e. the legs swing up too
quickly) and the body does not swing faster nor higher. 
...
\begin{thm}\label{thm:acrobot-energy-stabilization}
    Consider the acrobot ~\eqref{eqn:acrobot-hamiltonian}.
\begin{enumerate}
