...
find the diffeomorphism \(T\), and apply this lemma to prove the claims in
Theorem \ref{thm:acrobot-energy-stabilization}.
...
\ref{thm:acrobot-energy-stabilization}, we use
...
We omit the expressions for \(f_r(\theta,r,I)\) and \(f_\theta(\theta,r,I)\), as
they are too long for print.
...
The origin is now a repeller, which means there exists some
...
asymptotically stable equilibrium for \eqref{eqn:r-discrete} instead.
...
\subsection{All acrobots will rotate}
...
If the orbit \(x(t)\) does not hit the line \(|q_u| = \pi\), we now know it must
remain within the compact set \(\Sone \times [-K,K]\).
