...
\begin{abstract}
    ...
    acrobot, and prove rigorously that this constraint will inject or dissipate
    energy.
...
One can use the results of Theorem \ref{thm:acrobot-energy-stabilization} 
for energy regulation.
For example, one might employ a supervisor which switches
between injection and dissipation VNHCs to stabilize a desired energy
level \(E_\text{des}\), with some hysteresis to avoid infinite switching.
We will explore this idea in Section \ref{sec:simulations}.
...
\ref{fig:simple-acrobot-model}, because the torso and leg links have 
distributed mass. 
...
respectively, then the inertia matrix becomes
\begin{equation*}
    M(q) = \begin{bmatrix}
        m_{11}(q) & m_{12}(q) \\
        m_{12}(q) & m_{22}(q)
...
The dynamic parameters of Wang's acrobot can be found in Table
\ref{tab:acrobot-parameters}.
Evaluating the mechanical energy of an acrobot with these parameters
at the VNHC \(q_a = 0\) yields the mechanical energy of the nominal pendulum,
\[
    E(q_u,p_u) \approx 396.5501 p_u^2 + 0.5997(1 - \cos(q_u))
    .
\]

We will now simulate the effects of constraining Wang's acrobot with
...
overcome friction in the leg motor.
...
and experiments.
In other words, our injection VNHC is ~\eqref{eqn:acrobot-constraint} with 
\(I = 10\) while our dissipation VNHC is ~\eqref{eqn:acrobot-constraint} with 
\(I = -10\).
...
We simulated the acrobot constrained by our injection VNHC
with initial condition \((q_u,p_u) = \left(\pi/32,0 \right)\) for \(30\)
seconds, and plotted the resulting orbit in Figure \ref{fig:acrobot-in-orbit}.
...
We initialized our acrobot with a rotation at \((q_u,p_u) = (0,0.18)\) and
simulated the constrained dynamics (under the dissipation VNHC) for \(30\)
seconds.
...
Once can use a supervisor to stabilize oscillations by appropriately
...
One can also use a supervisor to stabilize rotations.
...
\(\delta = 0.02\).
The supervisor changed its choice of VNHC at each black star until it was within
range of \(p_\text{des}\), at which point it extended the leg (shown in red).

In Figure \ref{fig:acrobot-rot-reg-in} the acrobot was initialized at 
\((q_u,p_u) = (\pi/2,0)\); here the supervisor injected energy until
...
We performed the following tests on SUGAR.
...
\begin{figure}
    ...
    \caption{SUGAR's orbit during the baseline energy injection test.}
...
\begin{figure}
    ...
    \caption{SUGAR's orbit before (blue) and after (red) stopping.}
...
\item \textbf{Push Test:}
    ...
    it to rotate undisturbed for 15 seconds and then pushed it in its
    direction of motion.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{SUGAR's orbit before (blue) and after (red) pushing.}
\end{figure}

We had hoped to show more experimental results for dissipation and
oscillation/rotation regulation, but unfortunately the Dynamixel RX24F servo
...
SUGAR met an untimely end to its career. 
...
In this article we redeveloped the framework of virtual nonholonomic
constraints for underactuated mechanical systems.
We applied this framework towards the acrobot, and designed a constraint which
emulates giant motion from gymnastics.
...
Then we performed simulations of this VNHC, as well as physical experiments.
