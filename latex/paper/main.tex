...
\((A,B)\tpose \in \R^{(n+p)\times m}\) is the matrix obtained by stacking \(A\)
on top of \(B\). 
...
\begin{proof}
    ...
    \(\Sigma = (\diag{\sigma_1,\ldots,\sigma_k}, \Zmat{(n-k)\times k})\tpose\),
    \(\sigma_i > 0\), and \(U \in R^{n \times n}\),
    \(V \in \R^{k \times k}\) are unitary matrices \cite{calculating_svd}.
    Defining \(T = \diag{1/\sigma_1^2,\ldots,1/\sigma_n^2}\) and assigning the
...
\begin{proof}
    ...
    \(i,m \in \{1,\ldots,n\}\).
    Hence, \([\tilde{q}_i, \tilde{q}_j] = [\tilde{p}_i,\tilde{p}_j] = 0\).
    Then, since \(\mathbf{B}\) is orthogonal, 
    \((\mathbf{B}_i)\tpose (\mathbf{B}\tpose)_j = \delta_i^j\).
    Hence, the Poisson bracket between \(\tilde{p}_i\) and \(\tilde{q}_j\) is 
    \begin{align*}
        [\tilde{p}_i, \tilde{q}_j] &= \sum\limits_{m=1}^n A_{i,m}A_{j,m} 
                                   = (\mathbf{B}_i)\tpose (\mathbf{B}\tpose)_j
                                   = \delta_i^j
                                   .
    \end{align*}
    By (45.10) in \cite{landau_mechanics}, this is a canonical transformation,
    so the new Hamiltonian is
...
The nonholonomic constraint \(h(q,p)\) is \textit{virtual} because it is
enforced by the controller \(\tau(q,p)\), rather than arising from the physics
of the system.
...
Notice that if any component of \(e_i\) has relative degree 1, we may not be able
...
will have vector relative degree \(\{2,\ldots,2\}\).
In fact, this is a hard requirement: any VNHC which depends on the actuated
momentum cannot be regular. 
Before providing a characterization of regularity which proves this statement,
we define the following matrices as shorthand:
\begin{align}
...
\begin{assm}\label{assm:inertially-actuated}
    ...
    unactuated coordinates, so that 
