...
the nominal energy \(E(q_u,p_u)\) defined by \eqref{eqn:acrobot-energy}.
...
Thanks to the shape of a pendulum's orbits, we will show that (at \(I = 0\)) the
dynamics of \((r,\theta)\) satisfy \(\dot{r} = 0\) and \(\dot{\theta} > 0\).
...
\(t = t(\theta)\) to eliminate the dynamics of \(\theta\), and study how 
the pseudo-radius \(r\) evolves as a function of the pseudo-angle \(\theta\),
rather than as a function of time.
If it increases ``on average" , then \(E\) must also be increasing ``on average"
and the constrained dynamics must be gaining energy on \(D\).
...
\begin{lemma}
    ...
    let \(\underline{r}, \overline{r} \in \R_{> 0}\) with 
    ...
    \begin{align*}
        ...
        ,
    \end{align*}
    satisfying \(\dot{r} = 0\) and \(\dot{\theta} > 0\) at \(I = 0\).
    Let \(g(\theta,r,I) = \dot{r}/\dot{\theta}\) and assume that 
    \((\partial g/\partial I)(\theta,r,0) = 0\).
    Define \(b(\theta,r_0) = g(\theta,r_0,0)\) and assume there exists
    \(\epsilon > 0\) such that 
    \[
    ...
    gain energy on \(D\) for all \(0 < I \leq I^\star\), and lose energy on \(D\)
    for all \(-I^\star \leq I < 0\).
\end{lemma}
%/========== Conclusion ==========/%
