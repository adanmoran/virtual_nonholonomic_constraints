...
Let \(x(0) \in \mathcal{O}_1 \backslah \Pi^+\) and suppose that the orbit
\(x([0,\infty[)\) does not hit the line \(|q_u| = \pi\).
This orbit must remain in the compact set \(\Sone \times [-K,K]\) for some 
\(K > 0\).
Indeed, suppose otherwise.
Then for arbitrary \(K > 0\), there exists some \(t_K > 0\) such that
\(|p_u(t_K)| = K\), and there exists a \(t_{K/2}\) so that
\(|p_u(t)| >= K/2\) for \(t \in [t_K, t_K + t_{K/2}]\).
Looking at the constrained dynamics \eqref{eqn:acrobot-constrained-dynamics} in
this timespan, we see that \(\dot{p}_u \geq -3mgl\) and
\(\dot{q}_u \geq c_1 p_u + c_2 \geq c_1 K/2 + c_2\) for some constants
\(c_1, c_2 > 0\).
Picking \(K\) large enough guarantees that \(q_u\) covers a distance of \(2\pi\)
over this time interval, and so the orbit hits the line \(|q_u| = \pi\)
(thus providing a contradiction).

Thus, the orbit \(x(t)\) remains within the compact set \(\Sone \times [-K,K]\).
By the Birkhoff theorem, \(x(t)\) approaches a positive limit set, which is
either a closed orbit or the equilibrium \((\pi,0)\).

TODO: closed orbit case

We conclude that almost all orbits beginning in \(\mathcal{O}_1\) will, in
finite time, escape the closure of \(\mathcal{O}_1\) and begin rotating by
crossing the vertical line \(|q_u| = \pi\).
