...
also enables the acrobot to rotate around the bar a desired speed.
...
if
\[
    \lim\limits_{I \to 0} \frac{R(I)}{I} = 0
    .
\]
...
We temporarily take \(\mathcal{Q}\) to be embedded in \(\R^N\), so the
coordinate transformation used the following theorem is well-defined.
...
\(\mathcal{O}_2(\bar{\rho})\), which are sublevel sets of the nominal pendulum's
...
a diffeomorphism \((q_u,p_u) \leftrightarrow (r,\theta)\), where \(r\)
...
\(E\) is also be increasing on average.
This means the constrained dynamics must be gaining energy on \(D\).
...
%    with \(b(\theta,r_0) := (\partial g/\partial I)(\theta,r_0,0)\). 
    ...
    \[
        r_{n+1} \geq r_n + I\epsilon - \kappa I_2^2
        .
    \]
    Choosing \(\gamma > 0\) and \(I^\star \in \, ]0,I_2]\) small enough makes 
    \(I\epsilon - \delta I_2^2 \geq \gamma\) for all \(I \in ]0,I^\star]\).
...
\(\mathcal{O}_2(\bar{\rho})\) and apply Lemma \ref{lemma:energy-regulation-D}.
...
\(I_0 K Q^\prime(0) + (I_0)^2 \tilde{R}(I_0) > 0\), which means
...
\eqref{eqn:theta-dot-limit} of \(\dot{\theta}\) to \(\bar{\mathcal{O}}_1\)),
...
.
\]
We thus have \(\dot{\theta} > 0\) when \(I = 0\) as required.
Defining \(g(\theta,r,I) = \dot{r}/\dot{\theta}\), MATLAB symbolic computations
...
\(]\sqrt{60m^2gl^3},\bar{\rho}[\).
Thus, all requirements of Lemma \ref{lemma:energy-regulation-D} are satisfied,
so there exists a common \(I_2(\bar{\rho})\) whereby the acrobot gains/loses
energy on both \(\mathcal{R}^+\) and \(\mathcal{R}^-\).

Let \(I_1^\star\) be the parameter we found for \(\mathcal{O}_1\) in 
Section \ref{sec:proof-o1}.
Taking \(I^{\star\star} = \min \{I_1^\star, I_2(\bar{\rho})\}\),
the acrobot gains/loses energy on \(\mathcal{O}_2(\bar{\rho})\)
when \(|I| \leq I^{\star\star}\).
