...
\begin{tikzpicture}
    ...
    \draw[thick] (-2,0) coordinate (leftwall) -- (1+\len,0) coordinate (rightwall);
    ...
    \node[rotate=-\qu] (comone) at ($0.7*(m1)$) {$\centerofmass$};
    % TODO: The arrow showing l_1 and l_c1
    % The angle q_u
    ...
    \draw[draw=black, thick] ($(m2) + 0.2*(-\qas,\qac)$) -- 
        ($(m2) + 0.2*(\qas,-\qac)$);
    % The second center of mass somewhere along the rod
    \node[rotate=\qa] (comtwo) at ($(m1) + 0.5*(-\qac,-\qas)$) {$\centerofmass$};
    % TODO: THe arrow showing l_2 and l_c2
    % The angle q_a
    \pic [draw,<-, "$q_a$", angle eccentricity=1.5] {angle=m2--m1--gray};
