...
\begin{tikzpicture}
    ...
    \node[left] at ($(comone)+(0,-\len*0.05)$) {$m_1$};
    % The arrow showing l_1 and l_c1
    \draw[draw = black, ->|] (origin)++(\len*0.075,0) -- ($0.65*(m1)+(\len*0.075,0)$)
        node[fill=white,midway,above,sloped,inner sep=1pt] {$l_{c_1}$};
    \draw[draw=black, ->|] (origin)++(\len*0.25,0) -- ($0.9*(m1)+(\len*0.25,0)$)
        node[fill=white,midway,sloped,inner sep=0pt] {$l_1$};
    % The angle q_u
    ...
    \node[below] at ($(comtwo)+(0,-\len*0.05)$) {$m_2$};
    % The arrow showing l_2 and l_c2
    \draw[draw = black, |<->|] ($(m1)+(\len*0.05,\len*0.075)$) --
        ($(comtwo)+(0.025,\len*0.09)$)
        node[fill=white,pos=0.7,above,sloped,inner sep=1pt] {$l_{c_2}$};
    \draw[draw=black, |<->|] ($(m1)+(\len*0.15,\len*0.3)$) --
        ($(m2)+(\len*0.1,\len*0.32)$)
        node[fill=white,midway,sloped,inner sep=0pt] {$l_2$};
