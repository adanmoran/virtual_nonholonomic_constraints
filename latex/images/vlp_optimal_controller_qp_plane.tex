...
\begin{tikzpicture}
    ...
    \draw[draw=black, <->] (-4.5,0) -- (4.5,0) node[right] {$q$};
    \draw[draw=black, <->] (0,-4.5) -- (0,4.5) node[above] {$p$};
    % Draw an arc with the theta label up to some node in the plane
    \draw[draw=black] (0,0) -- (2,1) node[fill=black,inner sep= 1pt] {};
    \draw[draw = black] (1,0) arc (0:26:1) node at (1.2,0.25) {$\theta$};
\end{tikzpicture}
\end{document}
