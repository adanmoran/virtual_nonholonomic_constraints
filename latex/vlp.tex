...
therefore, \(h(\mathbf{q},\mathbf{p})\) is a regular VNHC whenever 
...
\begin{proof}
   ...
   \((q,p)\)-plane, so one can show it is equal to the sign of
   \(\sin(2\theta)\). 
   Furthermore, by Lemma \ref{lemma:sign-of-cube} we have
   \[ 
       \sign{l_\text{avg}^3 - L(\theta)^3} = \sign{l_\text{avg} - L(\theta)}
       .
   \]
   ...
   \begin{align*}
    \sign{\dot{E}_\text{avg}} &= \sign{\sin(q)p \left(L(\theta)^3 - l_\text{avg}^3\right)} \\
      ...
      .
   \end{align*}
   Thus, \(E_\text{avg}\) is a Lyapunov function and the negative-time system is
...
(instead of standing and squatting, resp.) produces the
