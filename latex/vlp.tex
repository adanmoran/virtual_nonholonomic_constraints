...
\begin{proof}
   ...
   \((\pi, 0)\) is given by
   \begin{equation}\label{eqn:linearized-vlp-neg-time}
   ...
   Taking the sublevel set
   \[
      E_{(\pi,0)} := \left\{ (q,p) \in \Gamma \mid E_\text{avg}(q,p) <
         E_\text{avg}(\pi,0) \right\}
      ,
   \] 
   it is clear form (\ref{eqn:vlp-proof-neg-inv}) that 
   \(\Pi_0^+ \subset E_{(\pi,0)}\)
   and 
   \(\Pi_0^- \subset \left(\Gamma \backslash E_{(\pi,0)} \right) \cup (\pi,0)\).
   Since only one eigenvalue of (\ref{eqn:linearized-vlp-neg-time})
   is negative, the stable manifold theorem tells us that both \(\Pi_0^+\) and
   \(\Pi_0^-\) are one-dimensional \cite{stable_manifold}.
