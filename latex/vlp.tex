...
\(\mathbb{S}^1 \times \R\), and the system has two equilibria,
...
sublevel sets on \(\mathbb{S}^1 \times \R\).  
...
largest subset of 
\( Z = \left\{(q,p) \in \mathbb{S}^1 \times \R \mid \dot{E}_\text{avg}(q,p) = 0
\right\} \). 
...
\[
   Z= \left\{(q,p) \in \mathbb{S}^1 \times \R \mid q =0\right\} 
   \cup \left\{(q,p) \in \mathbb{S}^1 \times \R \mid p =0\right\} 
   .
\]
...
time, i.e., that for each compact subset \(K\) of 
\(\mathbb{S}^1 \times \R\), and for almost every condition 
...
\(\mathbb{S}^1 \times \R\), and thus is a set of measure zero
\cite{stable_manifold}.

Let \( K \subset \mathbb{S}^1 \times \R\) be an arbitrary compact set and let 
