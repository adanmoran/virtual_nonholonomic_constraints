...
Before continuing, we must rigorously define what it means for a VNHC to inject
energy into the VLP.
We want the the VLP to behave in
such a way that momentum increases over time;
this intuition is captured mathematically by the following definitions.

\begin{defn}\label{defn:energy-injection}
   Let \(\dot{x} = f(x)\) be a planar ODE defined on \(D \subset \R^2\), with
   \(f(0) = (0,0)\).
   This system \textit{gains energy on \(D\)} if 
   \begin{enumerate}
...
\begin{defn}
   ...
   \textit{injects energy into the VLP} if the constrained dynamics gain energy
   everywhere on the constraint manifold, except possibly on a set of measure
   zero.
\end{defn}
...
\begin{proof}
   ...
   \begin{equation}\label{eqn:negative-time-vlp}
      ...
      .
   \end{equation}
   We begin by analyzing the upright equilibria \((\pm \pi, 0)\). 
   The linearization of (\ref{eqn:negative-time-vlp}) at these
   ...
   \[
      ...
      ,
   \]
   which has eigenvalues at \(\lambda = \pm \sqrt{\frac{g}{L(\pi)}}\).
   The positive-time system has the same eigenvalues at these upright
   equilibria.
   We label the set of initial conditions converging to \((\pm \pi, 0)\) in
   positive time by
   \[
      \Pi_0^+ := \left\{ (q_0,p_0) \in \Gamma \mid \lim_{t \to \infty} (q(t),p(t)) = (\pm \pi, 0) \right\}
      ,
   \]
   and the set of initial conditions converging to \((\pm \pi,0)\)
   in negative time by \(\Pi_0^-\).
   Since only one eigenvalue is negative, the stable manifold theorem tells us
   that both \(\Pi_0^+\) and \(\Pi_0^-\) are one-dimensional \textbf{SOURCE?},
   which means they have measure zero within \(\Gamma\).

   To prove the origin of the positive-time system is a repeller, we will show
   the origin of the negative-time system is almost globally
   ...
   For a proof by contradiction, suppose that for all \(T > 0\) there exists \(t > T\)
   where \(x(t) \in K\).
   Let 
   \[
      ...
      ,
   \] 
   be the largest energy value attained in \(K\), and let
   \[
      E_k := \left\{ (q,p) \in \Gamma \mid E_\text{avg}(q,p) \leq k \right\}
      ,
   \]
   be the energy sublevel set with that largest energy. 
   Note that \(E_k\) is compact, contains \(K\), and is negatively invariant (it
   is positively invariant for the negative-time system).
   ...
   all initial conditions) will not hit the upper equilibrium.
   ...
   by the Krasovskii-LaSalle arguments presented earlier, the VLP loses energy
   while converging asymptotically to \((0,0)\).
 \end{proof}
