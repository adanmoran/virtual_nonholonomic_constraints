...
by using the matrix Kronecker product (see Appendix \ref{apx:kron-prod}).
...
\[
    ...
    ,
\]
and assigning the regular feedback transformation\(\tau = V T \hat{\tau}\), 
...
Collecting the \(e_i\) together, we say that \(e\) is of relative degree
\(\{r_1,\ldots,r_k\}\).
...
We could guarantee \(h(q,p) \to 0\), but we could not in general guarantee 
\(\dot{h}(q,p) \to 0\). 
...
\begin{cor}\label{cor:2d-zero-dynamics}
    ...
    coordinates satisfying Assumption \ref{assm:inertially-actuated} 
    which has degree of underactuation one.
...
equations significantly reduces the complexity of analyzing large systems.
