...
by using the matrix Kronecker product.
In sum, the Hamiltonian system reduces to
...
\end{defn}
% TODO: delete this next definition and rephrase the text accordingly
\begin{defn}
...
\begin{proof}
    The change of coordinates \((Q = \mathbf{B}q, P = \mathbf{B}p)\) is a
    canonical transformation because it satisfies Lemma
    \ref{lemma:linear-is-canonical}.
    ...
    \[
        ...
        ,
    \]
    so \(\left(Q = (q_u,q_a), P = (p_u,p_a)\right)\) are simply actuated
    coordinates for \(\mathcal{H}\) as desired.
\end{proof}
