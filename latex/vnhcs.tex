...
For the mechanical systems of interest in this thesis, we assume that
...
system.  However, since \(\mathcal{Q}\) is a generalized cylinder,
\(\dot{q} \in \R^n\).
...
\begin{defn}
    A set of canonical coordinates \(\{q,p\}\) for a Hamiltonian system 
    \(\mathcal{H}\) are said to be \textit{simply actuated coordinates} if the
...
\begin{defn}
    Let \(\{q,p\}\) be canonical coordinates for a Hamiltonian mechanical system
...
\end{thm}
\begin{proof}
    The Poisson bracket between \(f(q,p)\) and \(g(q,p)\) is defined by
...
child will push off the ground and start a swing with a very small oscillation,
...
Unfortunately for engineers, reality does not provide ideal scenarios very often. 
...
motion accordingly and do not keep track of time when kicking their legs. This
...
and has even been used to design a snake-like robot that can swim
\cite{vhc_snake}.

Recent papers have attempted to extend these results to use the robot's
actuators to enforce a relation 
\(h(q,\dot{q}) = 0\) of the full state. Since these relations use both
the configuration and generalized velocity, they are called
\textit{nonholonomic} constraints. Furthermore, they are called \textit{virtual}
because the constraint is enforced by the robot's actuators.
We will give our own characterization of these virtual nonholonomic constraints
in the Hamiltonian framework, using the phase \((q,p)\) of a Hamiltonian
system instead of the state \((q,\dot{q})\) of a lagrangian system.
...
\begin{defn}
    ...
    stabilizing the set
    \[
        \Gamma = \left\{(q,p) \vert h(q,p) = 0, dh_q \dot{q} + dh_p \dot{p} = 0\right\}
    \]
    which is called the \textit{constraint manifold}.
\end{defn}

Defining the error term  \(e = h(q,p)\), stabilizing \(\Gamma\) is equivalent to 
solving for \(\tau(q,p)\) which drives \(e \rightarrow 0\)
and \(\dot{e} \rightarrow 0\).
...
invertible, which means that \(h(q,p)\) is strictly monotonic in \(p\).
...
one can use.
The terminology for this is to say that \(e\) is of \textit{relative degree}
\(\{2,2,\ldots,2\}\). We thus define a special type of VNHC which satisfies this
property.
...
\citeauthor{hybrid_zero_dynamics_bipedal_nhvcs} showed that relations which use only
the unactuated momentum \(p_u\) result in \(\tau\) appearing only after two
derivatives of \(e\) \cite{hybrid_zero_dynamics_bipedal_nhvcs}. 
In the remainder of this chapter, we will formalize and generalize this idea 
under the following assumption.

% TODO: Write the [p' M p] matrix in our nice nottation
\begin{assm}\label{assm:H-is-simply-actuated}
    The mechanical system under consideration is an underactuated 
    Hamiltonian system of order \((n-k) \geq 0\) in simply
    actuated coordinates \(\{q = (q_u,q_a), p = (p_u, p_a)\}\):
    \begin{align*}
...
% TODO: write out ??? = [p' dM/(dqu_i)] the matrix of size nxn
\begin{prop}
    A VNHC of order \(k\) is regular if and only if \(dh_{p_a} = 0\) 
    (\ie \(h(q,p) \equiv h(q,p_u)\)) and
    \[
        \rank{\left(dh_q \Minv(q) + dh_{p_u} ??? \right)\simpleB} = k
    \]
