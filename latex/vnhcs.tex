...
\begin{defn}\label{defn:energy-gain}
    Let \(M = \Rt{T_1} \times \cdots \times \Rt{T_n}\) 
    (where \(T_1, \ldots, T_n \in \{2\pi, \infty\}\)) be an
    \(n\)-dimensional generalized cylinder.
    Let \(f : M \rightarrow \R^n\) be smooth with \(f(0) = 0\)
    and let \(D \subset M\) be open.
    The system described by \(\dot{x} = f(x)\) 
    \textit{gains energy on \(D\)} if these conditions are satisfied:
    \begin{enumerate}
