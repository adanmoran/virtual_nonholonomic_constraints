...
\begin{align}\label{eqn:hamiltonian}
    \mathcal{H}(q,p) &= \frac{1}{2} p\tpose D^{-1}(q) p + P(q) \\
                     &\begin{cases}
        \dot{q} = D\inv(q)p \\
        \dot{p} = -\frac{1}{2} (I_n \otimes p\tpose) \nabla_q D\inv(q) p
        - \nabla_q P(q) + B(q) \tau \\
    \end{cases} 
\end{align}
...
From the proof for Theorem \ref{thm:vnhc-regularity}, a regular VNHC of order
\(k\) can be stabilized by the output-linearizing phase-feedback controller
(\ref{eqn:vnhc-torque-controller}).
% TODO: Controller for VNHC in closed form
\begin{equation}\label{eqn:vnhc-torque-controller}
    \tau(q,p) = ? - k_p e - k_d \dot{e}
\end{equation}
where \(k_p, k_d \in \R_{>0}\) can be tuned using standard linear control
techniques.
...
\begin{thm}\label{thm:zero-dynamics}
    ...
    \end{equation}
    where
    % TODO: This is too wide, shrink it somehow
    \begin{equation}\label{eqn:g-qpu}
        g(q_u,p_u) := \left.\left(dh_q \Minv(q) \simpleB \right)\inv 
