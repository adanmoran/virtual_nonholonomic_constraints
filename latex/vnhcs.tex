...
\begin{align*}
    \mathcal{H}(q,p) &= p\tpose D\inv(q)p - \frac{1}{2}p\tpose D\inv(q) p +
    P(q) \\
...
If there exists some \(\tau(q,p)\) driving \(e \to 0\) and \(\dot{e} \to 0\),
then \(\tau\) stabilizes \(\Gamma\). 
If \(\tau\) first appears after \(r_i > 0\) derivatives of \(e_i\), 
then \(e_i\) is said to have \textit{relative degree} \(r_i\).
We then say \(e\) has relative degree \(\{r_1,\ldots,r_k\}\).

With no further structure on \(h(q,p)\), the control input \(\tau\) usually
appears after one derivative. 
Unfortunately, if any \(e_i\) has relative degree \(1\) we may not in general
be able to stabilize \(\Gamma\); we could guarantee \(h(q,p) \to 0\), but not
necessarily \(\dot{h}(q,p) \to 0\). 
...
\(\Gamma\). 
This is a common requirement in the VHC literature, so we will make use of it
...
constraint as a \(C^2\) function 
\(h : \mathcal{Q} \times \mathcal{P} \rightarrow \R^k\) which has no singular
points on its constraint manifold
\[
    \Gamma = \left\{(q,p) \mid h(q,p) = 0, dh_q \dot{q} + dh_p \dot{p} = 0\right\}
    .
\]
...
To find the explicit equations for the dynamics on \(\Gamma\) of a regular VNHC, 
we made the following assumptions:
\begin{itemize}
    \item The inertia matrix satisfies 
        \(\nabla_{q_u}M(q) = \Zmat{n(n-k) \times n}\).
    \item On \(\Gamma\), one can solve linearly for \(q_a\) as a function of
        \((q_u,p_u)\).
\end{itemize}
...
Finally, the benefit of using VNHCs is that they reduce the
dimensionality of the system from \(2n\) equations of motion to \(2(n-k)\) of
them.
This can significantly reduce the complexity of analyzing the evolution of the
system.
