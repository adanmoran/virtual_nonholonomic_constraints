...
\citet{hybrid_zero_dynamics_bipedal_nhvcs} used this idea to define
the unactuated coordinate \(q_u = B^\perp q\), derive the unactuated momentum in
Lagrangian coordinates, and specify that a VNHC must use this unactauted
momentum.
These ideas are what inspired us to develop the theory of simply actuated
Hamiltonian systems, which is more suited to this area of research than
Lagrangian mechanics. 
Furthermore, our definition of a VNHC is more general than that of Griffin and
Grizzle.
They defined a VNHC as a relation \(h(q,\sigma_u)\) (where \(\sigma_u\) is the
Lagrangian mechanics proxy for \(p_u\)), while our definition of VNHCs allows
for relations that use the full phase.
...
\citet{hybrid_zero_dynamics_bipedal_nhvcs} were the first to derive the
constrained dynamics for VNHCs of the form \(h(q,\sigma_u)\).
In our derivation of the constrained dynamics, we use the same assumptions as
they do, with the additional assumption that one can solve for 
\(q_a = f(q_u,p_u)\) on \(\Gamma\).
While this means the constrained dynamics in
\cite{hybrid_zero_dynamics_bipedal_nhvcs} hold for a larger class of VNHCs, 
their derivations are terse and unclear.
By making this additional assumption, we were able to find a the constrained
dynamics in simply actuated coordinates, which are much more physically
intuitive.
Having the constrained dynamics in simply actuated coordinates will also
allow us to more easily study energy injection properties of VNHCs in the next
two chapters.
