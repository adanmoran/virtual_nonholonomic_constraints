%! TEX program = lualatex

\documentclass[12pt]{article}
\usepackage[utf8]{inputenc}

%/========== Preamble ==========/%

%----------- Choose Margins ----------%
\usepackage[margin=1in]{geometry}

%---------- Bibliography Style ----------%
\usepackage[style=ieee,backend=biber]{biblatex}
\addbibresource{bib.bib}

%----------- Math Packages -----------%
\usepackage{amsfonts}
\usepackage{mathtools}
\usepackage{amsthm} % allow theorems and definitions as a block

%% Define Theorem Styles %%
\theoremstyle{plain}
\newtheorem{thm}{Theorem}[section] % reset theorem numbering for each section
\newtheorem{lemma}[thm]{Lemma}
\newtheorem{prop}[thm]{Proposition}
\newtheorem*{cor}{Corollary}
%% Define Definition Styles %%
\theoremstyle{definition}
\newtheorem{defn}[thm]{Definition} % def'n number depends on section number
\newtheorem{exmp}[thm]{Example} % same for example numbers
\newtheorem{assm}[thm]{Assumption} % and for assumption numbers
%% Define Remark Styles %%
\theoremstyle{remark}
\newtheorem*{remark}{Remark}

%---------- Layout packages ----------%
\usepackage{minipage-marginpar}

%---------- Remove indenting from paragraphs ----------%
% Also makes each new paragraph start with an vertical space.
\usepackage[parfill]{parskip}

%---------- Custom Commands ----------%
\newcommand{\etal}{\textit{et al}. }
\newcommand{\ie}{\textit{i}.\textit{e}., }
\newcommand{\eg}{\textit{e}.\textit{g}. }
\newcommand{\Minv}{M^{-1}}

%/========== /Preamble ==========/%

%/========== Main Document ==========/%
\begin{document}

%---------- Title Page -----------%
\include{title}

%---------- Acknowledgements, TOC, Abstract, Intro -----------%
% TODO: all of this section

%---------- Main Content ----------%
TEST CITATIONS:

\cite{constrained_hamiltonian_systems}
\cite{control_giant_two_link_gymnastic_robot}
\cite{dynamical_servo_acrobot_vc}
\cite{dynamic_vhcs_stabilize_closed_orbits}
\cite{energy_pumping_robotic_swinging}
\cite{hamiltonian_nonholonomic_systems}
\cite{hamiltonization_of_nh_systems}
\cite{how_to_pump_a_swing}
\cite{hybrid_zero_dynamics_bipedal_nhvcs}
\cite{integrating_factors}
\cite{lagrangian_structure_reduced_dynamics_vhcs} 
\cite{multiple_lyapunov_functions}
\cite{multiple_lyapunov_functions_switched_systems}
\cite{nonholonomic_dynamics}
\cite{pendulum_length_giant_gymnastics}
\cite{pumping_swing_standing_squatting}
\cite{swingup_acrobot_pendulum}
\cite{swingup_giant_acrobot}
\cite{vhcs_for_el_systems}
\cite{vlp_energy_shaping}
\cite{xingbo_thesis}

% Notation page
\include{notation}
% Re-do VHCs from the Hamiltonian approach
\include{vhcs}
% Nonholonomic Constraints
\include{vnhcs}
% Discuss the variable length pendulum
\include{vlp}
% Give the Acrobot Model and Reduced Dynamics
\include{acrobot}

%---------- Bibliography ----------%
\newpage
\printbibliography
\end{document}
%/========== /Main Document ==========/%

% vim: set tw=80 ts=4 sw=4 sts=0 et ffs=unix :
