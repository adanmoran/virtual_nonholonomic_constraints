...
\definecolor{MM-blue}{HTML}{2883E6}
\newenvironment{MM}[1]{{\color{MM-blue}#1}}{}
...
\MM{\textbf{Related literature:} While the focus of this paper is on energy injection and dissipation, a closely related concept is that of orbital stabilization to a particular energy level set. The  problem of orbital stabilization has been investigated extensively and we mention four main approaches.
 ...
 There are similarities between the virtual constraint approach and the I\&I technique for orbital stabilization  proposed in~\cite{OrtYiRomAst20} in that both approaches seeks a controlled invariant submanifold on which the closed-loop system exhibit desired dynamics. But while in~\cite{OrtYiRomAst20} one wishes to obtain oscillator dynamics on the submanifold, in this paper we wish to obtain expansive or contractive dynamics (depending on the sign of a design parameter) which rules out oscillator dynamics.

In this paper, like in~\cite{YiOrtWuZha20}, we design a controller for energy pumping/damping, but the energy pumping/dumping mechanism proposed here does not arise from any form of energy shaping. In particular, it does not arise from an assignment of the damping matrix of the closed-loop system.
...
\MM{\(q=(q_u,q_a)\)} on the configuration manifold 
\(\mathcal{Q} = \Sone \times \Sone\), where \(q_u\) is unactuated and 
\(q_a\) is actuated.
We ignore dissipative forces in this model.

\MM{In what follows, we denote by \(c_u \eqdef \cos(q_u)\), \(c_a \eqdef \cos(q_a)\), and 
...
\MM{To this end, recalling the partitioning  \(q= (q_u,q_a)\), we define the conjugate of momenta, \(p = (p_u,p_a) = M(q)\dot{q}\) and the Hamiltonian function
...
theory of \vnhcs for a class of mechanical systems we
call ``simply actuated Hamiltonian systems". 
\MM{Although the results in Section \ref{sec:vnhc-vnhc} do not appear in the literature in the same form, we will show that} they are a special case of results that appeared in
\cite{hybrid_zero_dynamics_bipedal_nhvcs}.

\subsection{Simply Actuated Hamiltonian Systems} \label{sec:vnhc-sah}
\MM{Consider} a mechanical system modelled with generalized coordinates 
...
\begin{equation}\label{eqn:hamiltonian-eom-general}
    \begin{cases}
        \dot{q} = \nabla_p\mathcal{H} 
        , \\
        \dot{p} = -\nabla_q\mathcal{H} + \MM{B} \, \tau
        ,
    \end{cases}
\end{equation}
where \(\tau \in \R^k\) is a vector of generalized input forces and the input
matrix \MM{\(B \in \R^{n \times k}\) is assumed to be constant and of full rank \(k\).}
...
% \end{assm}
%
When \(\mathcal{Q} = \R^n\), \MM{one may} define a
canonical coordinate transformation of \eqref{eqn:hamiltonian} 
which decouples the input forces. To define this transformation we will make use of the following lemma\footnote{\MM{Lemma~\ref{lemma:B-orthogonal} and Theorem~\ref{thm:simply-actuated} are obvious, but we include their proofs for completeness.}}.
...
\MM{For} the Hamiltonian system \eqref{eqn:hamiltonian} with configuration
    manifold \(\mathcal{Q} = \R^n\)\MM{,
    the} coordinate transformation
    \((\tilde q,\tilde p) = \left(\mathbf{B}\tpose)\inv q,\mathbf{B}p\right)\)
    is a canonical transformation. \MM{ the} resulting \MM{Hamiltonian function is 
    \begin{equation}
...
\MM{of the form~\eqref{eq:Hamiltonian_system:simply_actuated}} a 
...
representation of these constrained dynamics for \vnhcs with a specific form.
...
degree of underactuation one. \MM{The recent paper~\cite{SimStraBloCol23} has initiated a systematic geometric investigation of \vnhcs that are affine functions of the configuration velocities. The ones used in this paper are nonlinear functions of the configuration velocities.}
...
\begin{defn}
    A \vnhc \(h(q,p) = 0\) of order \(k\) is \textit{regular} if \MM{system~\eqref{eq:Hamiltonian_system:simply_actuated}} with output 
    \(e = h(q,p)\)  \MM{has vector} relative degree \(\{2,2.\ldots,2\}\) everywhere on the
    constraint manifold \(\Gamma\).
\end{defn}

\MM{The reader is referred to~\cite{Isi95} for the definition of vector relative degree.}
The authors of
...
Under additional mild conditions (see \cite{vhcs_for_el_systems}), \MM{the constraint manifold associated with } a regular \vnhc of
order \(k\) can be \MM{asymptotically} stabilized by the input-output feedback linearizing
controller
...
\textbf{Comparison with existing \MM{virtual constraint} literature}: Horn \etal provide the constrained
